...
public:
    ...
    void start(){
        ...
        m_reconnect_thread_ = std::thread(
            [this](){
                ...
                while(ros::ok() && !reconnecting.load()){
                ...
                ROS_INFO_STREAM("websocket Reconnect thread exiting");
            }
        );
    ...
    void connect(){
        ...
        if (ec)
        {
            ROS_ERROR_STREAM("Error: " << ec.message());
            return;
        }
    ...
    void send_message(const std::string message){
        // ros_INFO_STREAM(ros::get_logger("WebSocketClient"), "Sending message: " << message);
